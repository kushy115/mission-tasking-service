...
EXPORT_FORMATS: dict[str, ExportSpec] = {
    "kml": ExportSpec("kml", "kml", "application/vnd.google-earth.kml+xml",
                     "Google Earth / Mission Planner KML"),
    "gpx": ExportSpec("gpx", "gpx", "application/gpx+xml",
                     "GPS Exchange Format (universal)"),
    "dji": ExportSpec("dji", "csv", "text/csv",
                     "DJI Waypoint v1 (Litchi/DJI Fly compatible)"),
}
...
def plan_to_dji(plan: MissionPlan) -> str:
    ...
    for _leg_num, _leg_type, _wp_idx, wp in _iter_waypoints(plan):
        lines.append(
            f"{wp.lat:.6f},{wp.lon:.6f},{wp.alt_m:.1f},0,0.2,0,"
            f"0,0,-1,0,0,5.0,0,0,0,0"
        )
    return "\n".join(lines) + "\n"
